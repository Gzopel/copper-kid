...
const parsePlayerName = (nodes) => {
  if (nodes.length === 1) { // us
    return nodes[0].data.trim();
  }
  return nodes[1].children[0] && nodes[1].children[0].data; // player on vacation behaves differently :/
}

const parseGalaxyPlanets = (html) => {
...
export const galaxyPageParser = (page) => {
  ...
  return parseGalaxyPlanets(root);
};
