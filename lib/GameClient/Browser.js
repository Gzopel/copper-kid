...
export class Browser {
  ...
  invest = (page, code, token) => {
    const url = `https://${this.server}/game/index.php?page=${page}&modus=1&type=${code}&menge=1&token=${token}`;
    return new Promise((resolve, reject) => {
      console.log('getting url', url)
      request.defaults({
        headers: this._getGetHeaders(),
  ...
  // https://s142-en.ogame.gameforge.com/game/index.php?page=defense&deprecated=1
  // token:13dfeed83a74bcb91a2f84d7b08488ee
  // modus:1
  // type:502
  // menge:
  ...
  _doGetLogin = (url) => {
    return new Promise((resolve, reject) => {
      ...
      request.defaults({
        ...
        this.lastRequest = url;
        if (error) {
          console.error('rejecting _doGetLogin', error)
          reject(error);
        }
        this.cookieMonster.parseAndAddCookies(response.headers['set-cookie']);
  ...
  _doGetReLogin = (url) => {
    return new Promise((resolve, reject) => {
      ...
      request.defaults({
        ...
        this.lastRequest = url;
        if (error) {
          console.error('rejecting _doGetReLogin', error)
          reject(error);
        }
        this.cookieMonster.parseAndAddCookies(response.headers['set-cookie']);
        resolve(body);
