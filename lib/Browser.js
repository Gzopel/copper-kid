...
export class Browser {
  ...
  getView(page, planet) {
    planet = planet || this.lastPlanet;
    const url = `https://s142-${this.server}/game/index.php?page=${page}&cp=${planet}`;
  ...
  invest(page, planet, code) {
    return new Promise((resolve, reject) => {
      ...
      return getView(page, planet)
        .then(resolve)
        .catch(reject);
    }).then(this._doInvest(page, code))
      .then(PARSERS[page]);
  }

  _doInvest = () => {
    const url = `https://s142-${this.server}/game/index.php?page=${page}&modus=1&type={code}&menge=1`;
    return new Promise((resolve, reject) => {
      console.log('getting url', url)
      request.defaults({
        headers: {
